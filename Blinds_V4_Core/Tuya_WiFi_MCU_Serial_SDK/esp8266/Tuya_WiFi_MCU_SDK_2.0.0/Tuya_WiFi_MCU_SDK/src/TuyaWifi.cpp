...
void TuyaWifi::data_handle(unsigned short offset)
{
  ...
  case GET_ONLINE_TIME_CMD: // Get system time (Greenwich Mean Time)
    tuya_extras.mcu_get_green_time((unsigned char *)(tuya_uart.wifi_data_process_buf + offset + DATA_START), (TUYA_WIFI_TIME*)&_green_time);
    break;
#endif

#if SUPPORT_RTC_TIME
  case GET_LOCAL_TIME_CMD: // Get local time
    tuya_extras.mcu_get_rtc_time((unsigned char *)(tuya_uart.wifi_data_process_buf + offset + DATA_START), (TUYA_WIFI_TIME*)&_rtc_time);
    break;
#endif
...
char TuyaWifi::get_green_time(TUYA_WIFI_TIME *time, const unsigned int timeout)
{
  ...
  while (millis() < (get_green_time_begin + timeout))
  {
    ...
    { /* request green time success */
      tuya_tools.my_memcpy(time, (const void*)&_green_time, sizeof(_green_time));
...
char TuyaWifi::get_rtc_time(TUYA_WIFI_TIME *time, const unsigned int timeout)
{
  ...
  while (millis() < (get_rtc_time_begin + delay_time))
  {
    ...
    { /* request rtc time success */
      tuya_tools.my_memcpy(time, (const void*)&_rtc_time, sizeof(_rtc_time));
